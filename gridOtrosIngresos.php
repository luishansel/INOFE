...
<?php }
	else
	{
		...
		$PermisoUsuario = fxPermisoUsuario("procOtrosIngresos", $mbAgregar, $mbModificar, $mbBorrar, $mbAnular);
...
<?php }
		else
		{
			if (isset($_POST["KDSA"]))
            {
                fxAnularOtrosIngresos($_POST["KDSA"]);
    ...
    <div id="DivContenido">
        <div class="row">
            <div class="col-md-12">
                ...
                <table id="grid" class="table table-condensed table-hover table-striped" data-selection="true"
                    data-multi-select="false" data-row-select="true" data-keep-selection="true">
                    <thead>
                        <tr>
                            ...
                            <th data-column-id="NOMBRE_040" data-order="asc" data-align="left" data-header-align="left">Nombre</th>
                    ...
                    <tbody>
                        <?php
							$mDatos = fxDevuelveOtrosIngresos();
...
$(function() {
    ...
    $("#append").on("click", function() {
        $.redirect("procOtrosIngresos.php", "POST");
    });
    ...
    $("#remove").on("click", function() {
        if ($.trim($("#grid").bootgrid("getSelectedRows")) != "")
        {
            ...
            $.redirect("gridOtrosIngresos.php", {KDSA: msCodigo}, "POST");
        }
    });
    ...
    $("#view").on("click", function() {
        ...
        {
            var msCodigo = $.trim($("#grid").bootgrid("getSelectedRows"));
            $.redirect("procOtrosIngresos.php", {mAccion: 1, mCodigo: msCodigo}, "POST");
        }
    });
});
</script>
