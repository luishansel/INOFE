...
<?php }
	else
	{
		...
		$PermisoUsuario = fxPermisoUsuario("hrrCfgModulo", $mbAgregar, $mbModificar, $mbBorrar, $mbAnular);
		...
		else
		{
            ...
            {
                $Codigo = ($_POST["KDSA"]);
                fxBorrarConfModulos($Codigo);
                fxAgregarBitacora($_SESSION["gsUsuario"], "KDSA110A", $Codigo, "", "Borrar");
            }
		?>
    	<div class="container">
        	<div id="DivContenido">
        	<div class="row">
            	<div class="col-md-12">
                    ...
                    <table id="grid" class="table table-condensed table-hover table-striped" data-selection="true" data-multi-select="false" data-row-select="true" data-keep-selection="true">
                    	<thead>
                            <tr>
                                <th data-column-id="CFGMODULO_REL" data-identifier="true" data-align="left" data-width="15%">Configuración</th>
                                <th data-column-id="CURSO_110" data-align="left" data-header-align="left">Curso asociado</th>
                            </tr>
                        </thead>
                        <tbody>
                        <?php
							$m_cnx_MySQL = fxAbrirConexion();
							$mDatos = fxDevuelveConfModulos(1);
							
							while ($Fila = $mDatos->fetch())
							{
								echo ("<tr>");
								echo ("<td>" . $Fila["CFGMODULO_REL"] . "</td>");
                                echo ("<td>" . $Fila["CURSO_110"] . "</td>");
								echo ("</tr>");
							}
						}
						?>
...
<script>
        $(function() {
			...
			$("#append").on("click", function() {
				$.redirect("hrrCfgModulo.php", "POST");
			});

            $("#edit").on("click", function() {
				if ($.trim($("#grid").bootgrid("getSelectedRows")) != "")
				{
					var codConfiguracion = $.trim($("#grid").bootgrid("getSelectedRows"));
					$.redirect("hrrCfgModulo.php", {KDSA: codConfiguracion}, "POST");
				}
            });
			...
			$("#remove").on("click", function() {
				...
				{
					var codConfiguracion = $.trim($("#grid").bootgrid("getSelectedRows"));
					$.redirect("gridCfgModulo.php", {KDSA: codConfiguracion}, "POST");
				}
			});
        });
    </script>
