...
else
{
	...
	if (isset($_POST["msModulo"]))
		$codModulo = $_POST["msModulo"];
	else
		$codModulo = "";
	...
	if ($codCurso != "")
	{
		$msConsulta = "select CLASE_REL, FECHACLASE_130, NOMBRE_020, FECHAINI_020, FECHAFIN_020, HORAINI_020, HORAFIN_020, fxDevuelveDias(KDSA020A.CURSO_REL) as DIASCLASE, ";
		...
		$msConsulta .= "and KDSA021A.CURSO_REL = KDSA020A.CURSO_REL and KDSA021A.CURSO_REL = ? order by FECHACLASE_130";
		$m_Principal = $m_cnx_MySQL->prepare($msConsulta);
		$m_Principal->execute([$codCurso]);
	}
	else
	{
		$msConsulta = "select CLASE_REL, FECHACLASE_130, NOMBRE_020, FECHAINI_020, FECHAFIN_020, HORAINI_020, HORAFIN_020, fxDevuelveDias(KDSA020A.CURSO_REL) as DIASCLASE, ";
		$msConsulta .= "CONVOCATORIA_020, GRUPO_020, TIPOASISTENCIA_020, NOMBRE_100 ";
		$msConsulta .= "from KDSA130A, KDSA021A, KDSA020A, KDSA100A where KDSA130A.MODULO_REL = KDSA021A.MODULO_REL and KDSA021A.DOCENTE_REL = KDSA100A.DOCENTE_REL ";
		$msConsulta .= "and KDSA021A.CURSO_REL = KDSA020A.CURSO_REL and KDSA021A.MODULO_REL = ? order by FECHACLASE_130";
		$m_Principal = $m_cnx_MySQL->prepare($msConsulta);
		$m_Principal->execute([$codModulo]);
	}
	...
	while ($Fila = $m_Principal->fetch())
	{
		$codPlanClase = $Fila["CLASE_REL"];
		...
		$pdf->Curso = utf8_decode($Curso);
		$pdf->Vigencia = "Del " . DevuelveFecha($FechaIni) . " al " . DevuelveFecha($FechaFin);
		$pdf->DiasClase = $DiasClase;
		$pdf->Horario = "De " . date_format($HoraIni, 'h:i a') . " a " . date_format($HoraFin, 'h:i a');
		$pdf->Convocatoria = $Convocatoria . ' / G' . $Grupo;
		$pdf->FechaClase = DevuelveFecha($FechaClase);
		$pdf->Docente = utf8_decode($Docente);
