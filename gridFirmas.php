...
<?php }
	else
	{
		...
		$PermisoUsuario = fxPermisoUsuario("catFirmas", $mbAgregar, $mbModificar, $mbBorrar, $mbAnular);
		...
		if (isset($_POST["KDSA"]))
            {
                fxBorrarFirma($_POST["KDSA"]);
				fxAgregarBitacora($_SESSION["gsUsuario"], "KDSA008A", $_POST["KDSA"], "", "Borrar");
            }
		?>
    	<div class="container">
        	<div id="DivContenido">
        	<div class="row">
            	<div class="col-md-12">
                    ...
                    <table id="grid" class="table table-condensed table-hover table-striped" data-selection="true" data-multi-select="false" data-row-select="true" data-keep-selection="true">
                    	<thead>
                            <tr>
                                <th data-column-id="FIRMA_REL" data-identifier="true" data-align="left">Autorizado</th>
                                <th data-column-id="NOMBRE_008" data-align="left">Nombre</th>
								<th data-column-id="CARGO_008" data-align="left">Cargo</th>
                            </tr>
                        </thead>
                        <tbody>
                        <?php
							$Datos = fxDevuelveFirma();
							
                            while ($Fila = $Datos->fetch())
							{
								echo ("<tr>");
								echo ("<td>" . $Fila["FIRMA_REL"] . "</td>");
								echo ("<td>" . $Fila["NOMBRE_008"] . "</td>");
								echo ("<td>" . $Fila["CARGO_008"] . "</td>");
								echo ("</tr>");
							}
						}
						?>
...
<script>
        $(function() {
			...
			$("#append").on("click", function() {
                $.redirect("catFirmas.php", "POST");
			});
  
			$("#remove").on("click", function() {
                if ($.trim($("#grid").bootgrid("getSelectedRows")) != "")
                {
                    var codFirma = $.trim($("#grid").bootgrid("getSelectedRows"));
                    $.redirect("gridFirmas.php", {KDSA: codFirma}, "POST");
                }
			});
            ...
            $("#edit").on("click", function() {
                ...
                {
                    var codFirma = $.trim($("#grid").bootgrid("getSelectedRows"));
                    $.redirect("catFirmas.php", {KDSA: codFirma}, "POST");
                }
            });
        });
    </script>
