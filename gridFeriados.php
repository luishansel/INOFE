...
<?php }
	else
	{
		...
		$PermisoUsuario = fxPermisoUsuario("hrrFeriados", $mbAgregar, $mbModificar, $mbBorrar, $mbAnular);
		...
		if (isset($_GET["KDSA"]))
            {
                fxBorrarFeriado($_GET["KDSA"]);
				fxAgregarBitacora($_SESSION["gsUsuario"], "KDSA001A", $_GET["KDSA"], "", "Borrar");
            }
		?>
    	<div class="container">
        	<div id="DivContenido">
        	<div class="row">
            	<div class="col-md-12">
                    ...
                    <table id="grid" class="table table-condensed table-hover table-striped" data-selection="true" data-multi-select="false" data-row-select="true" data-keep-selection="true">
                    	<thead>
                            <tr>
                                <th data-column-id="FECHA_001" data-identifier="true" data-align="center" data-header-align="center" data-width="15%">Fecha</th>
                                <th data-column-id="DIA_001" data-align="left" data-header-align="left" data-width="10%">Día</th>
								<th data-column-id="DESC_001" data-align="left" data-header-align="left">Descripción</th>
                            </tr>
                        </thead>
                        <tbody>
                        <?php
							$mDatos = fxDevuelveFeriado(1);
							
                            while ($Fila = $mDatos->fetch())
							{
                                echo ("<tr>");
                                $fecha = date_create_from_format('Y-m-d', $Fila["FECHA_001"]);
								echo ("<td>" . date_format($fecha, 'd-m-Y') . "</td>");
								echo ("<td>" . $Fila["DIA_001"] . "</td>");
								echo ("<td>" . $Fila["DESC_001"] . "</td>");
								echo ("</tr>");
							}
						}
						?>
...
<script>
        $(function() {
			...
			$("#append").on("click", function() {
				$.redirect("hrrFeriados.php", "POST");
			});
  
			$("#remove").on("click", function() {
            	if ($.trim($("#grid").bootgrid("getSelectedRows")) != "")
				{
                    var codFeriado = $.trim($("#grid").bootgrid("getSelectedRows"));
                    $.redirect("gridFeriados.php", {KDSA: codFeriado}, "POST");
                }
            });
            ...
            $("#edit").on("click", function() {
				...
				{
                    var codFeriado = $.trim($("#grid").bootgrid("getSelectedRows"));
                    $.redirect("hrrFeriados.php", {KDSA: codFeriado}, "POST");
                }
            });
        });
    </script>
