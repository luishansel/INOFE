...
<?php }
	else
	{
		...
		$PermisoUsuario = fxPermisoUsuario("procAsistencia", $mbAgregar, $mbModificar, $mbBorrar, $mbAnular);
		...
		else
		{
			if (isset($_POST["KDSA"]))
            {
				$mnResultado = fxBorrarAsistencia($_POST["KDSA"]);
				fxAgregarBitacora($_SESSION["gsUsuario"], "KDSA140A", $_POST["KDSA"], "", "Borrar");
            }
		?>
    	<div class="container">
        	<div id="DivContenido">
				...
				<div class="row">
					<div class="col-md-12">
						...
						<table id="grid" class="table table-condensed table-hover table-striped" data-selection="true" data-multi-select="false" data-row-select="true" data-keep-selection="true" style="font-size:small">
							<thead>
								<tr>
									<th data-column-id="ASISTENCIA_REL" data-identifier="true" data-order="desc"  data-align="left" data-header-align="left" data-width="12%">Asistencia</th>
									<th data-column-id="NOMBRE_021" data-align="left" data-header-align="left">Curso - Módulo</th>
									<th data-column-id="FECHA_140" data-align="center" data-header-align="center" data-width="12%">Fecha de clase</th>
								</tr>
							</thead>
							<tbody>
							<?php
								...
								while ($Fila = $mDatos->fetch())
								{
									...
									$fecha = date_create_from_format('Y-m-d', $Fila["FECHA_140"]);
...
<script>
	...
	$(function() {
		...
		$("#edit").on("click", function() {
			...
			{
				var codAsistencia = $("#grid").bootgrid("getSelectedRows");
				$.redirect("procAsistencia.php", {KDSA: codAsistencia[0]}, "POST");
			}
		});
		...
		$("#remove").on("click", function() {
			...
			{
				var codAsistencia = $("#grid").bootgrid("getSelectedRows");
				$.redirect("gridAsistencia.php", {KDSA: codAsistencia[0]}, "POST");
			}
		});
