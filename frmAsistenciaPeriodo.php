		...
		<?php }
			...
			<div class = "row">
				<div class="col-xs-12 col-md-11">
					<div class="degradado"><strong>Asistencias de los docentes</strong></div>
				</div>
			</div>
                    ...
                    <form name="frmAsistenciaPeriodo" id="frmAsistenciaPeriodo" onsubmit="return verificarFormulario()">
                        ...
                        <div class = "row">
                            <div class="col-auto col-xs-offset-none col-md-4 col-md-offset-2">
								...
								<input type="button" id="Exportar" name="Exportar" value="Exportar a Excel" class="btn btn-warning" />
                            </div>
                        </div>
                    </form>			
			<?php	}
...
<script type='text/javascript'>
	function verificarFormulario()
	{
		...
		{
			$.messager.alert('INOFE','La Fecha Inicial es mayor que la Final.','warning');
			return false;
		}
	...
	$("#Imprimir").on("click", function() {
		if (verificarFormulario)
			$.redirect("repAsistenciaPeriodo.php", {dtpFechaIni: document.getElementById('dtpFechaIni').value, dtpFechaFin: document.getElementById('dtpFechaFin').value}, "POST", "_blank");
	});

	$("#Exportar").on("click", function() {
		if (verificarFormulario)
			$.redirect("exlAsistenciaPeriodo.php", {dtpFechaIni: document.getElementById('dtpFechaIni').value, dtpFechaFin: document.getElementById('dtpFechaFin').value}, "POST", "_blank");
	});
</script>